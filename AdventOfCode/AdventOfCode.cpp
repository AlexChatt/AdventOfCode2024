// AdventOfCode.cpp : This file contains the 'main' function. Program execution begins and ends there.
//

#include "SolutionFunctions.h"

int main()
{
	//DayOneSolution("input1.txt");
	//DayTwoSolution("input2.txt");
	//DayThreeSolution("input3.txt");
	//DayFourSolution("input4.txt");
	//DayFiveSolution("input5.txt");
	//DaySixSolution("input6.txt");
	//DaySevenSolution("input7.txt");
	//DayEightSolution("input8.txt");
	//DayNineSolution("input9.txt");
	//DayTenSolution("input10.txt");
	//DayElevanSolution("input11.txt");
	//DayTwelveSolution("input12.txt");
	//DayThirteenSolutions("input13.txt");
	//DayFourteenSolution("input14.txt");
	//DayFiveteenSolution("input15.txt");
	DaySixteenSolution("input16.txt");
}